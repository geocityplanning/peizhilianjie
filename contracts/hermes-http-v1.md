...
| `input.resource_fallback_page` | 资源不足兜底页 | Excel 中的资源兜底页字段；必填。执行端仅在唯一可见复制对话框中以原生输入填写、框架组件 value/model 回读一致且切换页签往返后仍一致；保存后须先在唯一主表同一行精确核验新增应用 ID、应用名、渠道三锚（各匹配列必须唯一），再从该行打开复制对话框，对兜底字段执行两次稳定 DOM/model 回读一致时才可返回成功。复制表单的应用名/渠道不作为持久化身份锚。 |
