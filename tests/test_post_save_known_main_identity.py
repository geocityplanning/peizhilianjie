...
def test_known_locator_zero_candidate_is_bounded_read_only_polling(monkeypatch):
    ...
    assert sum(page.waits) == cap._POST_SAVE_KNOWN_ID_VISIBILITY_WAIT_BUDGET_MS
    assert len(page.waits) == cap._POST_SAVE_KNOWN_ID_POLL_ATTEMPTS - 1


def test_known_locator_visibility_wait_budget_preserves_three_full_scans(monkeypatch):
    cap = _cap()
    elapsed_scan_ms = [0]
    scans = []
    ...
    def scan(*args):
        scans.append(True)
        elapsed_scan_ms[0] += cap._POST_SAVE_KNOWN_ID_VISIBILITY_WAIT_BUDGET_MS + 1
    ...
    assert result["reason"] == "zero_candidates_after_poll"
    assert len(scans) == cap._POST_SAVE_KNOWN_ID_POLL_ATTEMPTS
    assert elapsed_scan_ms[0] > cap._POST_SAVE_KNOWN_ID_VISIBILITY_WAIT_BUDGET_MS
